...
async def process_donations(bot):
    ...
    for it in items:
        ...
        with Session() as db:
            kv_mode = db.get(KV, f"mode:{tg_id}")
            mode = kv_mode.value if kv_mode else "outline"
        ...
        if mode == "mobile":
            ...
            vless_uri = s.mobile_vless_uri or "Мобильный режим временно недоступен — свяжитесь с поддержкой."
            text = (
                "✅ Оплата получена. Доступ активирован (📱 *Мобильный режим*).\n\n"
                "Установите клиент и импортируйте ссылку:\n"
                "• Android: v2rayNG\n"
                "• iOS: FoXray / Sing-box\n"
                "• Windows: v2rayN\n"
                "• macOS: Stash\n\n"
            ...
            try:
                ...
                log.error("Send VLESS to user failed: %s", e)
            ...
            with Session() as db:
                ...
                access_url = user.current_key_secret
                db.commit()
            ...
            try:
                await bot.send_message(
                    chat_id=tg_id,
                    text=(
                        "✅ Оплата получена. Доступ активирован (⚡ *Outline*).\n\n"
                ...
                log.error("Send Outline key to user failed: %s", e)
